...
const { Voter, Bulletin, Keys, Generator } = require('../models');
...
function reconstructOriginal(parsedInput) {
  ...
  if (
    ...
    const dictEntries = Object.entries(parsedInput[1])
      .map(
        ([key, value]) =>
          `${JSON.stringify(key)}: ${processElementTuple(value)}`
      )
      .join(', ');
    ...
    const tuples = parsedInput
      .map((tuple) => processElementTuple(tuple))
      .join(', ');
    return `[${tuples}]`;
  }
  ...
  throw new Error(
    'Unrecognized input format: ' + JSON.stringify(parsedInput, null, 2)
  );
}
...
function handleParsedResult(result, res) {
  ...
  function parseNested(item) {
    if (typeof item === 'string') {
      ...
      return Object.fromEntries(
        Object.entries(item).map(([key, value]) => [key, parseNested(value)])
      );
    }
...
router.post('/pf_zksm_verf', async (req, res) => {
  try {
    ...
    if (!result1) {
      return res
        .status(422)
        .send({ error: 'Error during generation of signatures' });
    }
    ...
    for (const [electionId, electionData] of Object.entries(formattedResult)) {
      ...
      const result2 = await callPythonFunction2(
        'pf_zksm',
        Verfpk,
        Sigs,
        EncSigs,
      ...
      const result3 = await callPythonFunction(
        'verfsmproof',
        Verfpk,
        Sigs,
        EncSigs,
      ...
      allElectionResults[electionId] = {
        encrypted_votes: parsedResult[0],
        results: parsedResult[1],
        status_forward_set_membership: parsedResult[2],
      };
    }
...
router.post('/pf_zkrsm_verf', async (req, res) => {
  try {
    ...
    for (const [electionId, electionData] of Object.entries(formattedResult)) {
      console.log(`Processing election ${electionId}`);
      const { verfpk, sigs_rev, enc_sigs_rev, enc_sigs_rev_rands } =
        electionData;
      const Verfpk = reconstructOriginal(verfpk);
      ...
      const parsedResult = JSON.parse(result3);
      allElectionResults[electionId] = {
        decrypted_votes: parsedResult[0],
        results: parsedResult[1],
        status_reverse_set_membership: parsedResult[2],
      };
    }
    //console.log("here")
...
router.post('/fetch', async (req, res) => {
  ...
  try {
    ...
    if (!checkVoter || checkVoter.length === 0) {
      return res
        .status(422)
        .send({ error: 'This voter_id is not in the voter list' });
    }
    ...
    if (!Bullet || Bullet.length === 0) {
      return res
        .status(422)
        .send({ error: 'This voter_id has not voted in the election' });
    }
...
router.post('/vvpat', async (req, res) => {
  try {
    ...
    if (result === "This VVPAT doesn't correspond to a decrypted vote.") {
      ...
      return res
        .status(500)
        .json({ error: 'Unexpected response from verification process.' });
    }
  } catch (err) {
...
router.post('/runBuild2', async (req, res) => {
  try {
    ...
    const appPath = path.join(
      '/app/evoting_localstorage/BallotAudit/android/app/build/outputs/apk/release',
      'app-release.apk'
    );
...
router.post('/runBuild3', async (req, res) => {
  try {
    // Path to the pre-existing app file
    const appPath = path.join(
      '/app/evoting_localstorage/VoterVerification/android/app/build/outputs/apk/release',
      'app-release.apk'
    );

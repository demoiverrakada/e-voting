...
const VerifySetMembership = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await axios.post("https://7000-01kk9e5t37w5v48yspx6hdpj3s.cloudspaces.litng.ai/pf_zksm_verf", {}, {
      ...
      {results && Object.entries(results).map(([electionId, electionResult]) => (
        <div key={electionId} className="election-result">
          ...
          <div className="forward-set-membership">
            <h4>Overall Status: {electionResult.status_forward_set_membership ? 'True' : 'False'}</h4>
          </div>

          {/* Votes table */}
          {electionResult.encrypted_votes && electionResult.results && (
            <div className="result-table">
              ...
              <table className="table">
                ...
                <tbody>
                  {electionResult.encrypted_votes.map((vote, index) => (

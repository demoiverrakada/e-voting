...
def bbspluskeygen(election_id):
    _sk = group.random(ZR)
    g1,f2 = load("generators",["g1","f2"],election_id).values()
...
def bbsplusquasibatchverify(sigmas, comms, pk,election_id):
    ...
    g1,f1,f2,h1,ef1f2 = load("generators",["g1","f1","f2","h1","ef1f2"],election_id).values()
